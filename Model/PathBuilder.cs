...
internal class PathBuilder(ILogger<PathBuilder> logger, Map map)
{
    ...
    {
        var target = movable.Path.TargetPoint;

        var sourceRect = map[movable.Id];
        var points = new List<Point>();
        while (true)
        {
            var result = EvadeObjects(sourceRect, movable.Id, target, out var altTarget);
            if (result == EvasionResult.AltTarget)
            {
                points.Add(altTarget);
                sourceRect = new Rect(altTarget, movable.Size);
            }
            else
            {
                // TODO: Find a way out
                // TEMP
                if (result == EvasionResult.Fail)
                {
                    logger.LogError("Failed to evade objects");
                }
                // movable.Path.New(map[movable.Id].Pos, map.RandomFreeRect(movable.Size).Pos);

                break;
            }

            movable.Path.Populate(points);
        }
    }

    private EvasionResult EvadeObjects(Rect movableRect, int movableId, Point absTarget, out Point altTarget)
    {
        altTarget = absTarget;
